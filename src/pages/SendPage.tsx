...
const SendPage = ({ formData, videoBlob, onBack, onComplete }: SendPageProps) => {
  ...
  const sendToTelegram = async (type: 'запись' | 'задача' | 'брак' | 'брак2' = 'запись') => {
...
✨ Отправлено через IMPERIA PROMO APP`;
  ...
  const sendViaTelegramBot = async (message: string, video: Blob, type: 'запись' | 'задача' | 'брак' | 'брак2') => {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4">
      <div className="text-center space-y-8 max-w-md mx-auto">
        ...
        <div className="space-y-4">
          ...
          <Button 
            onClick={() => sendToTelegram('брак2')}
            disabled={isSending}
            size="lg"
            className="w-full text-lg px-8 py-6 h-auto bg-yellow-600 hover:bg-yellow-700"
          >
            {isSending ? (
              <>
                <Icon name="Loader2" size={20} className="mr-2 animate-spin" />
